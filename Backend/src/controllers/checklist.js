...
//aqui vai faazer a verificaco da consulta antes de salvar assim garantindo que o medico so vai conseguir salvar checklists em consultas que pertencem a ele 
    try {
        ...
        if (consulta.length === 0) {
            return res.status(403).json({ mensagem: 'Consulta não encontrada ou não pertence a você.' });
        }
// aqui é a verificaco de duplicidade ou seja cada consulta so vai ter uma check list, se tentar cadastrar duas, vai retornar 401 e dar erro
        ...
        if (existente.length > 0) {
            return res.status(409).json({ mensagem: 'Checklist já cadastrado para essa consulta.' });
        }
        ...
        await pool.query(
            ...
            [
                ...
                sin_movimentos_repetitivos || 0, sin_frouxidao || 0,
                sin_macroquidia || 0, sin_face_alongada || 0
            ]
        );
