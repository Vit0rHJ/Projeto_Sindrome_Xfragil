...
const gerarLaudo = async (req, res) => {
    ...
    try {
        ...
        if (checklist.length === 0) {
            return res.status(404).json({ mensagem: 'Checklist não encontrado. Realize o checklist antes de gerar o laudo.' });
        }
...
//aqui é aonde vamos escrever o conteudo do pdf, da pra define fonte texto e pa
        ...
        doc.fontSize(12).text('Sistema de Identificação da Síndrome do X Frágil', { align: 'center' });
        doc.moveDown();
        ...
        doc.text(`Data: ${new Date(consulta[0].data_consulta).toLocaleDateString('pt-BR')}`);
        ...
        doc.text(`Encaminhamento: ${checklist[0].encaminhamento.replace('_', ' ').toUpperCase()}`);
        ...
        doc.fontSize(10).text(`Documento gerado em ${new Date().toLocaleString('pt-BR')}`, { align: 'center', color: 'gray' });
//aqui ele vai servir pra sinalizar que acabamos de escrever, depois do    fazer a parada dele vai salvar no baco e enviar para o usuario baixar 
        ...
        stream.on('finish', async () => {// vai esperarar o arquivo terminar de ser gravado no disco, é importante pra nao mandar um arquivo incompleto
            await pool.query(
