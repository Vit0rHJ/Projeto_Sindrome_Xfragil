...
const cadastrarMedico = async (req, res) => {
    ...
    if (!nome || !email || !senha || !crm || !especialidade) {// verifica se os campos obrigatorios foram enviados
        return res.status(400).json({ mensagem: 'Todos os campos são obrigatórios.' });
    }
  try { 
    const [ existente ] = await pool.query('SELECT * FROM usuarios WHERE email = ?', [email] // antes dele realizar o cadastro vai verificar se ja existe alguem com esse gmail, podemos fazer éssa verificacao atraves so do id mesmo

    );
    if (existente.length > 0) { // se encontra ao menos um resultado é pq o email ja esta em uso
...
module.exports = { cadastrarMedico, listarMedicos };